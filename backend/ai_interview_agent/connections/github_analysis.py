...
class GitHubAnalyzer:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    url_user = f"{self.base_url}/users/{os.getenv('GITHUB_USERNAME', '')}/repos"
    ...
    async def run_analysis(self, username: str, org: str = None) -> dict:
        ...
        commit_summary_text = (
            f"{username} owns {', '.join(top_services) if top_services else 'no repositories'} "
            f"with {len(all_commits)} commits in the last 6 months."
        )
    ...
    def _generate_groq_summary(self, content: str) -> str:
        ...
        prompt_template = ChatPromptTemplate.from_messages([
            ("system", (
                "You are an expert engineering analyst. Summarize the raw GitHub history "
                "of a departing engineer. Identify core components, dependencies, "
                "and list out technical elements or fragile areas an AI interviewer should "
                "ask them about during an offboarding interview.\n\n"
...
async def main():
    ...
    target_user = os.getenv("GITHUB_USERNAME", "your-github-username")
